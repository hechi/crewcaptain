...
@WebMvcTest(controllers = [ReviewPacketController::class])
...
class ReviewPacketControllerTest {
    ...
    class TestConfig {
        @Bean
        fun reviewPacketPort(): ReviewPacketPort = mockk()
    ...
    @Autowired
    private lateinit var reviewPacketPort: ReviewPacketPort
    ...
    fun `should return 401 when not authenticated`() {
        mockMvc.perform(
            get("/api/v1/persons/$testPersonId/review-packet")
    ...
    fun `should return markdown content with correct headers`() {
        val markdown = "# Review Packet: Jane Smith\n\n## Executive Summary\n"
        every { reviewPacketPort.generateReviewPacket(any()) } returns markdown
        ...
        mockMvc.perform(
            ...
            .andExpect(header().string("Content-Disposition", "attachment; filename=\"review-packet.md\""))
            .andExpect(content().string(markdown))
    }

    @Test
    fun `should return 404 when person not found`() {
        every { reviewPacketPort.generateReviewPacket(any()) } throws
    ...
    @Test
    fun `should return 400 when dateFrom is missing`() {
        mockMvc.perform(
            get("/api/v1/persons/$testPersonId/review-packet")
                .param("dateTo", "2024-06-30")
                .with(authentication(authenticatedJwt()))
    ...
    @Test
    fun `should return 400 when dateTo is missing`() {
        mockMvc.perform(
            get("/api/v1/persons/$testPersonId/review-packet")
                .param("dateFrom", "2024-01-01")
                .with(authentication(authenticatedJwt()))
    ...
    fun `should pass date range parameters to service`() {
        val querySlot = slot<GenerateReviewPacketQuery>()
        every { reviewPacketPort.generateReviewPacket(capture(querySlot)) } returns "# Review Packet"
    ...
    @Test
    fun `should scope review packet by authenticated userId`() {
        val querySlot = slot<GenerateReviewPacketQuery>()
        every { reviewPacketPort.generateReviewPacket(capture(querySlot)) } returns "# Review Packet"
    ...
    @Test
    fun `should return 400 when dateTo is before dateFrom`() {
        every { reviewPacketPort.generateReviewPacket(any()) } throws
            IllegalArgumentException("dateTo must not be before dateFrom")

        mockMvc.perform(
            get("/api/v1/persons/$testPersonId/review-packet")
                .param("dateFrom", "2024-06-30")
