...
- [x] Self-Assigned Quick Notes ("My Notes") — Managers can create personal quick notes (not linked to any person) via a new `selfAssigned` boolean flag. Invariant: selfAssigned and personId are mutually exclusive — assigning to a person clears selfAssigned. Backend: Flyway migration (self_assigned column + partial index), domain model updated with markSelfAssigned() method and init invariant, service/controller/DTOs extended with selfAssigned field and filter. Frontend: "My Notes" page at /my-notes accessible via user dropdown menu, dedicated create form (auto-sets selfAssigned=true), status filter (All/Active/Archived), archive/delete actions. API client updated with selfAssigned query parameter. Full test coverage: domain (5 new tests), service (4 new tests), controller (3 new tests), integration (5 new tests), frontend page (12 tests), API client (2 new tests), Navigation (1 new test). (2026-05-15)
...
- Frontend: 952 total — component tests (including OneOnOneActionItems 15 tests), page tests (including CreateOneOnOneEntryPage action items 1 test), API client tests (including action items originatingEntryId 1 test), API proxy route tests, auth token refresh tests, middleware tests, Navigation test (last run: 2026-05-15)
